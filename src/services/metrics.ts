...
export class MetricsService {
   ...
   * 记录用户行为
   */
  public recordUserBehavior(userId: string, eventType: EventType, properties: any = {}): void {
  ...
  public calculatePlanProgressRate(): MetricData {
    ...
    const progressRate = totalPlans > 0 ? (completedPlans / totalPlans) * 100 : 0;
  ...
  public calculateAISuggestionAdoptionRate(): MetricData {
    ...
    const adoptionRate = totalSuggestions > 0 ? (adoptedSuggestions / totalSuggestions) * 100 : 0;
  ...
  public calculateAITriggerRate(): MetricData {
    ...
    const triggerRate = totalUsers > 0 ? (usersWithAIInteraction / totalUsers) * 100 : 0;
  ...
  public calculateNotificationOpenRate(): MetricData {
    ...
    const openRate = totalNotifications > 0 ? (openedNotifications / totalNotifications) * 100 : 0;
  ...
  public getAllMetrics(): Record<MetricType, MetricData> {
    return {
      ...
      [MetricType.AI_SUGGESTION_ADOPTION]: this.calculateAISuggestionAdoptionRate(),
      ...
      [MetricType.REMINDER_EFFECTIVENESS]: this.calculateReminderEffectiveness()
    };
  }
  ...
  private calculateReminderEffectiveness(): MetricData {
    ...
    const effectiveness = totalReminders > 0 ? (effectiveReminders / totalReminders) * 100 : 0;
