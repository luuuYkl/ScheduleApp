 ...
 * @returns AI 复盘内容
 */
export async function generateAIReview(request: ReviewRequest): Promise<AIReview> {
  ...
  // 构建 AI 提示
  const prompt = buildPrompt(period, filteredTasks, filteredSchedules, metrics, context);
 ...
 * 计算坚持度评分
 */
function calculateConsistencyScore(tasks: Task[], schedules: ScheduleItem[]): number {
  if (tasks.length === 0 && schedules.length === 0) return 0;

  // 统计每天的完成情况
  const dailyCompletion: Map<string, { total: number; done: number }> = new Map();
...
function parseReviewResponse(
  ...
  try {
    ...
    const parsed = jsonMatch ? JSON.parse(jsonMatch[0]) : parseSimpleFormat(content);
