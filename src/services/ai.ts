...
import type { AIOptimizePlanRequest, AIOptimizePlanResponse, AISuggestion, AIRecommendedTask } from "./api.types";
...
async function sendAIRequest(body: Record<string, any>, label: string): Promise<AIRequestResult> {
  try {
    ...
    if (!response.ok) {
      console.error(`AI API 请求失败(${label}):`, response.status, response.statusText, text);
...
function parseAIContent(content: string, request: AIOptimizePlanRequest): AIOptimizePlanResponse | null {
  console.log('========== AI 解析开始 ==========');
  ...
  console.error('清理后内容:\n', cleaned);
  console.error('提取的JSON:', jsonMatch ? jsonMatch[0].substring(0, 500) : '无法提取');
  console.error('====================================');
  ...
  return {
    suggestions: [
      {
        type: "info",
        message: "AI 返回了非标准格式的内容，请查看浏览器控制台获取完整响应。点击「应用优化建议」按钮查看 Mock 建议。",
      },
    ],
    optimized_plan: {
      title: undefined,
 ...
 * 生成模拟建议（当 AI 不可用时）
 */
function generateMockSuggestions(request: AIOptimizePlanRequest): AIOptimizePlanResponse {
  ...
  if (lowerTitle.includes("学习") || lowerTitle.includes("study") || lowerTitle.includes("learn")) {
    addTask("每日学习 60 分钟", { start_time: "07:30", note: "晨间高效时段", repeat_type: "daily" });
    addTask("完成练习/项目实践", { start_time: "20:00", note: "巩固所学", repeat_type: "daily" });
    addTask("整理学习笔记", { task_date: end_date, note: "复盘整合" });
  } else if (lowerTitle.includes("健身") || lowerTitle.includes("运动") || lowerTitle.includes("锻炼")) {
  ...
  } else if (lowerTitle.includes("阅读") || lowerTitle.includes("read") || lowerTitle.includes("书")) {
    addTask("每日阅读 30 页", { start_time: "21:00", repeat_type: "daily" });
    addTask("撰写阅读笔记", { task_date: end_date, note: "输出关键观点" });
  } else if (lowerTitle.includes("工作") || lowerTitle.includes("项目") || lowerTitle.includes("project")) {
    addTask("拆分任务与排期", { task_date: start_date, note: "明确优先级" });
    addTask("每周里程碑检查", { start_time: "09:00", repeat_type: "weekly", repeat_end_date: end_date });
    addTask("风险清单更新", { start_time: "17:00", repeat_type: "weekly", repeat_end_date: end_date });
  } else {
    addTask("将目标拆解为可执行子项", { task_date: start_date });
    addTask("设置阶段性检查点", { repeat_type: "monthly" });
    addTask("记录执行中的问题与改进", { repeat_type: "daily" });
    addTask("定期回顾并调整", { repeat_type: "weekly", repeat_end_date: end_date });
  }
 ...
 * 快速验证计划合理性（不调用 AI）
 */
export function quickValidatePlan(request: AIOptimizePlanRequest): AISuggestion[] {
  const suggestions: AISuggestion[] = [];
