...
import { useNotification, NotificationOptions, ReminderConfig } from './notification';
...
export class ReminderService {
  ...
  public scheduleDailyCheckinReminder(
    ...
    const [hour, minute] = this.advanceNotice.dailyCheckin.split(':').map(Number);
  ...
  private getNextActiveTime(baseTime: Date): Date {
    ...
    // 找到下一个活跃时段
    const timeSlots = [
      { start: this.activeTimes.morningStart, end: this.activeTimes.morningEnd },
      { start: this.activeTimes.afternoonStart, end: this.activeTimes.afternoonEnd },
      { start: this.activeTimes.eveningStart, end: this.activeTimes.eveningEnd }
    ];
  ...
  private isInActiveTime(time: string): boolean {
    return (
      (time >= this.activeTimes.morningStart && time <= this.activeTimes.morningEnd) ||
      (time >= this.activeTimes.afternoonStart && time <= this.activeTimes.afternoonEnd) ||
      (time >= this.activeTimes.eveningStart && time <= this.activeTimes.eveningEnd)
    );
  }
   ...
   * 获取下一个指定星期几的时间
   */
  private getNextWeekday(weekday: 'Monday' | 'Tuesday' | 'Wednesday' | 'Thursday' | 'Friday' | 'Saturday' | 'Sunday', hour: number, minute: number): Date {
