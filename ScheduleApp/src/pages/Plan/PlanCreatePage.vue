...
async function createPlan() {
  console.log("[PlanCreatePage] createPlan 触发");
  console.log("[PlanCreatePage] 表单数据:", { ...form });
  
  if (!validate()) {
    console.log("[PlanCreatePage] 验证失败");
    return;
  }

  // 检查用户登录状态
  console.log("[PlanCreatePage] 用户状态:", userStore.user);
  
  if (!userStore.user?.id) {
    console.error("[PlanCreatePage] 用户未登录，userStore.user:", userStore.user);
    alert("请先登录");
    router.push("/login");
    return;
  }
  ...
  console.log("[PlanCreatePage] 准备创建计划，payload:", payload);

  try {
    if (editId.value) {
      if (!planStore.updatePlan) throw new Error("更新接口未实现");
      console.log("[PlanCreatePage] 更新现有计划，ID:", editId.value);
      await planStore.updatePlan(editId.value, payload);
      console.log("[PlanCreatePage] 计划更新成功");
    } else {
      if (!planStore.createPlan) throw new Error("创建接口未实现");
      console.log("[PlanCreatePage] 创建新计划");
      const newPlan = await planStore.createPlan(payload);
      console.log("[PlanCreatePage] 计划创建成功:", newPlan);

      // 如果有待创建的推荐任务，自动创建它们
      if (pendingTasks.value.length > 0 && newPlan.id && userStore.user?.id) {
        console.log("[PlanCreatePage] 创建推荐任务，数量:", pendingTasks.value.length);
    ...
    console.log("[PlanCreatePage] 准备跳转到首页");
    // 跳回计划概览页面（PlanOverview.vue 在 /home）
    router.push("/home");
  } catch (e: any) {
    console.error("[PlanCreatePage] 计划保存失败:", e);
