...
export const useUserStore = defineStore("user", () => {
  ...
  async function restore() {
    try {
      console.log("[user.store] 开始恢复用户会话...");
      
      // 已有用户和 token，直接返回（避免重复请求）
      if (user.value && token.value) {
        console.log("[user.store] 内存中已有用户:", user.value);
        return user.value;
      }
      ...
      if (!tk) {
        console.log("[user.store] 没有 token，用户未登录");
        return null;
        const config = await import("@/config");
        if (config.APP_CONFIG.USE_MOCK_API) {
          console.log("[user.store] Mock 模式，自动登录默认用户");
          // 尝试登录默认用户
          try {
            const defaultUser = await login("demoUser", "anyPassword");
            console.log("[user.store] 默认用户登录成功:", defaultUser);
            return defaultUser;
          } catch (loginError) {
            console.warn("[user.store] 默认用户登录失败:", loginError);
          }
        }
        return null;
      }
      ...
      if (user.value) {
        console.log("[user.store] 从存储中恢复用户:", user.value);
        return user.value;
      }
