    ...
    <transition name="menu-fade">
      ...
      >
        <button 
          ref="taskButton"
          class="fab-menu-item" 
          @click.stop="handleAction('task', $event)"
          @keydown.enter="handleAction('task')"
          @keydown.space.prevent="handleAction('task')"
          aria-label="新建任务"
          tabindex="0"
        >
...
const taskButton = ref<HTMLButtonElement | null>(null)
...
function handleAction(action: 'task' | 'plan' | 'schedule', event?: Event) {
  ...
  setTimeout(() => {
    ...
    console.log(`[FAB] Navigating to ${targetRoute}`)
    try {
