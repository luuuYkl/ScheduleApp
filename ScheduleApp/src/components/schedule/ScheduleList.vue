    ...
    <ul v-else>
      <li v-for="item in items" :key="item.id" class="schedule-item">
        <div class="title-row">
          ...
          <small v-if="item.start_time"
            >{{ item.start_time }} - {{ item.end_time || "未结束" }}</small
          >
        </div>
...
const items = computed(() =>
  scheduleStore.schedules.filter((s) => s.date === props.date),
);

async function reload() {
  await scheduleStore.load(props.date);
}
